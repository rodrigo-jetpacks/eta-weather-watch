...
const Index = () => {
  ...
  const handleCreateWeatherAlert = (alertData: Omit<WeatherAlert, 'id' | 'createdAt'>) => {
    ...
    const affectedShipments = shipments.filter(
      shipment => 
        shipment.destinationMetroCode === alertData.metroCode && 
    ...
    const newWeatherEvents: WeatherEvent[] = affectedShipments.map(shipment => {
      ...
      switch (alertData.severity) {
        case 'low': delayHours = 12; break;
        case 'medium': delayHours = 24; break;
        case 'high': delayHours = 48; break;
        case 'critical': delayHours = 72; break;
      }
      ...
      return {
        ...
        impactLevel: alertData.severity === 'low' ? 'minor' : 
                   alertData.severity === 'medium' ? 'moderate' :
                   alertData.severity === 'high' ? 'major' : 'severe',
        delayHours,
        createdAt: new Date().toISOString(),
    ...
    setShipments(prev => prev.map(shipment => {
      if (affectedShipments.find(affected => affected.id === shipment.id)) {
        return { ...shipment, status: 'delayed' as const };
      }
      return shipment;
    }));
